...
def add_to_history(well_name, horizon, forecast_combined, forecast_test, forecast, model_name, rmse, mae, mape, fig_original, 
...
if selected_model == "Chronos":
    ...
    #device_map_chronos = st.sidebar.selectbox('Устройство для Chronos', ['cpu'], index=['cpu'].index(st.session_state.device_map_chronos))
    ...
    chronos_model_name = st.sidebar.text_input('Имя модели Chronos', st.session_state.chronos_model_name)
...
if selected_model == "TFT":
    ...
    tft_max_epochs = st.sidebar.slider('Max Epochs TFT', 10, 100, 50)
...
if selected_model == "N-BEATS":
    ...
    nbeats_max_epochs = st.sidebar.slider('Max Epochs N-BEATS', 10, 100, 50)
...
if st.sidebar.button("Автонастройка"):
    for well_name in selected_wells:
        ...
        significant_regressors = [
            ...
            if stats.spearmanr(df_train[regressor], df_train[target_parameter])[1] < 1.00
        ]  
    def objective(trial):
        ...
        if selected_model == "DLT":  # Если выбрана модель DLT
            ...
            # TimeSeriesDataSet для тренировки
            training = TimeSeriesDataSet(
                ...
                time_varying_known_reals=["time_idx"] + significant_regressors,
                time_varying_unknown_reals=[target_parameter],
                target_normalizer=GroupNormalizer(groups=["well_id"], transformation="softplus"),
            ...
            model = Trainer(max_epochs=5, accelerator="auto")
            ...
            nbeats = NBeats.from_dataset(
                training,
                learning_rate = trial.suggest_loguniform("learning_rate", 1e-4, 1e-1),
                #widths = trial.suggest_categorical("widths", [[32, 512], [64, 256], [128, 256]]),
                #backcast_loss_ratio = trial.suggest_uniform("backcast_loss_ratio", 0.5, 1.5),
                #loss=QuantileLoss(),
                #weight_decay = trial.suggest_loguniform("weight_decay", 1e-5, 1e-2),
            ).float()
            
            trainer = Trainer(max_epochs=5, accelerator="auto", gradient_clip_val=0.01)
    ...
    if selected_model == "DLT":
        ...
        st.session_state.tft_learning_rate = best_params.get("learning_rate", 0.03)
        st.session_state.tft_hidden_size = best_params.get("hidden_size", 16)
        st.session_state.tft_attention_head_size = best_params.get("attention_head_size", 2)
        st.session_state.tft_dropout = best_params.get("dropout", 0.1)
        st.session_state.tft_hidden_continuous_size = best_params.get("hidden_continuous_size", 8)
        st.session_state.tft_max_epochs = best_params.get("max_epochs", 50)
    elif selected_model == "N-BEATS":
        st.session_state.nbeats_learning_rate = best_params.get("learning_rate", 0.01)
        st.session_state.nbeats_widths = best_params.get("widths", [32, 512])
        st.session_state.nbeats_max_epochs = best_params.get("max_epochs", 50)
        st.session_state.nbeats_backcast_loss_ratio = best_params.get("backcast_loss_ratio", 1.0)
    ...
    for well_name in selected_wells:
        ...
        if selected_model == "DLT":
            model = DLT(
                ...
                regressor_col=significant_regressors,
            ...
            df_train_prophet = df_train.rename(columns={"ds": "ds", target_parameter: "y"})
            df_test_prophet = df_test.rename(columns={"ds": "ds"})
            model.fit(df_train_prophet)
            ...
            # Настройки для TFT
            max_encoder_length = 168  # Охват недели (настраивается)
            max_prediction_length = horizon  # Будущий прогноз
            ...
            # Модель
            nbeats = NBeats.from_dataset(training, learning_rate=nbeats_learning_rate).float()
        ...
        # Обработка прогнозов для всех моделей
        if selected_model == "Prophet":
            forecast_values = forecast_test['yhat'].values
            real_values = df_test[target_parameter].values
        ...
        elif selected_model in ["DLT", "KTR", "ETS", "LGT"]:
        ...
        if selected_model == "LSTM":
            ...
            future_dates = pd.date_range(df_well.index[-1] + pd.DateOffset(days=1), periods=horizon)
            ...
            # Прогноз на будущее
            future_dates = pd.date_range(df_well['ds'].iloc[-1] + pd.DateOffset(days=1), periods=horizon, freq='D')
            future_df = pd.DataFrame({
                'ds': future_dates,
                'time_idx': range(df_well['time_idx'].max() + 1, df_well['time_idx'].max() + 1 + horizon)
            })

            for regressor in significant_regressors:
                if regressor in df_well.columns:
                    future_df[regressor] = df_well[regressor].iloc[-1]

            future_df['well_id'] = str(well_name)
            future_df[target_parameter] = np.nan

            try:
                forecast_future = tft.predict(future_df, mode="prediction")
                forecast_future = pd.DataFrame({
                    'ds': future_dates,
                    'yhat': forecast_future.flatten()[:len(future_dates)]
                })
            except Exception as e:
                st.error(f"Ошибка при прогнозировании TFT: {e}")
                forecast_future = pd.DataFrame({'ds': future_dates, 'yhat': [np.nan] * len(future_dates)})

            st.write(f"TFT: Проверка forecast_future: {forecast_future.head()}")
            ...
            # Прогноз на будущее
            future_dates = pd.date_range(df_well['ds'].iloc[-1] + pd.DateOffset(hours=1), periods=horizon, freq='H')
            future_df = pd.DataFrame({
                'ds': future_dates,
                'time_idx': range(df_well['time_idx'].max() + 1, df_well['time_idx'].max() + 1 + horizon)
            })
            ...
            try:
                forecast_future = nbeats.predict(future_df, mode="prediction")
                forecast_future = pd.DataFrame({
                    'ds': future_dates,
                    'yhat': forecast_future.flatten()[:len(future_dates)]
                })
            except Exception as e:
                st.error(f"Ошибка при прогнозировании N-BEATS: {e}")
                forecast_future = pd.DataFrame({'ds': future_dates, 'yhat': [np.nan] * len(future_dates)})

            st.write(f"N-BEATS: Проверка forecast_future: {forecast_future.head()}")
        ...
        # Убедитесь, что forecast — это DataFrame
        #if not isinstance(forecast, pd.DataFrame):
            #forecast = pd.DataFrame({'ds': df_future['ds'].values, 'prediction': forecast})
        ...
        if selected_model == "Prophet":
            ...
            fig_original = go.Figure()
            fig_original.add_trace(go.Scatter(x=df_well_original.index, y=df_well_original[target_parameter], mode='lines', name='Реальные данные', line=dict(color=graph_color_actual, width=graph_line_width)))
            fig_original.add_trace(go.Scatter(x=forecast_test['ds'], y=forecast_test['yhat'], mode='lines', name='Прогноз на реальных данных', line=dict(color=graph_color_test, width=graph_line_width)))
            fig_original.add_trace(go.Scatter(x=forecast['ds'], y=forecast['yhat'], mode='lines', name='Прогноз на будущее', line=dict(color=graph_color, width=1)))
            fig_original.update_layout(
       ...
       # ))
        ...
        add_to_history(
            well_name, horizon, forecast_combined, forecast_test, forecast, selected_model, rmse, mae, mape, fig_original, 
            target_parameter, estimator_dlt, estimator_lgt, estimator_ets, estimator_ktr, global_trend_option_dlt, 
            n_bootstrap_draws_dlt, n_bootstrap_draws_lgt, n_bootstrap_draws_ets, n_bootstrap_draws_ktr, 
            regression_penalty_dlt, regression_penalty_lgt, num_steps_ktr, fill_method, seasonality_mode, 
            chronos_model_name, device_map_chronos, lstm_epochs, lstm_batch_size, lstm_n_steps, lstm_n_length,
